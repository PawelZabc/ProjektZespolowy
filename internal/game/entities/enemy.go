...
func (e *Enemy) Move() {
	if e.Target != nil {
		...
		if rl.Vector2Length(rl.Vector2Subtract(*e.Target, physics.GetVector2XZ(e.Collider.GetPosition()))) < 0.5 {
			e.Target = nil
		}
...
func (e *Enemy) UpdateTarget(players []*Player, cols *[]colliders.Collider) {
	...
	for i, player := range players {
		...
		if cylinder, ok := player.Collider.(*colliders.CylinderCollider); ok {
			playerHeight = cylinder.Height
		}
		if cylinder2, ok := e.Collider.(*colliders.CylinderCollider); ok {
			enemyHeight = cylinder2.Height
		}
		ray := colliders.Ray{Origin: rl.Vector3Add(e.Collider.GetPosition(), rl.NewVector3(0, enemyHeight, 0)),
